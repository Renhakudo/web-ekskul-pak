...
import { Loader2, Fingerprint, Calendar, Copy, RefreshCw, CheckCircle2, History, X, Trash2, Download } from 'lucide-react'
...
export default function AdminAbsensiPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    const { data: cls } = await supabase.from('classes').select('id, title').order('title')
    if (cls) {
      setClasses(cls)
  ...
  const fetchHistory = async (classId: string) => {
    ...
    const { data, error } = await supabase.from('attendances')
      .select(`
        ...
        profiles!attendances_user_id_profiles_fk ( full_name ),
  ...
  const handleDeleteAttendance = async (id: string) => {
    if (!confirm('Hapus riwayat absensi ini? Batas absensi siswa ini akan di-reset.')) return
  ...
  const exportToCSV = () => {
    ...
    let csvContent = "Nama Siswa,Tanggal,Waktu,Status,XP\n"
    
    history.forEach(h => {
      const name = `"${h.profiles?.full_name || 'User Tidak Diketahui'}"`
      const date = new Date(h.created_at).toLocaleDateString('id-ID', {day: '2-digit', month: '2-digit', year:'numeric'})
      const time = new Date(h.created_at).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'})
      const status = h.status || 'hadir'
      const xp = h.xp_awarded || 0
      csvContent += `${name},${date},${time},${status},${xp}\n`
    })
    ...
    link.setAttribute('download', `Absensi_Kelas_${new Date().toISOString().split('T')[0]}.csv`)
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedClassId} onValueChange={setSelectedClassId}>
                  <SelectTrigger className="h-14 border-4 border-slate-900 font-bold bg-white text-base rounded-2xl focus:ring-0">
                    <SelectValue placeholder="Memuat kelas..." />
                  </SelectTrigger>
          ...
          <Card className="border-4 border-slate-900 shadow-[6px_6px_0px_#0f172a] rounded-[2rem] overflow-hidden h-full">
            <div className="bg-slate-900 p-4 text-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={exportToCSV} size="sm" className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold border-2 border-transparent hover:border-slate-900 transition-all rounded-lg">
                  ...
                  {history.map((h, i) => (
                    <div key={h.id} className="bg-white border-2 border-slate-900 rounded-xl p-4 flex items-center justify-between shadow-[2px_2px_0px_#0f172a] hover:shadow-[4px_4px_0px_#0f172a] hover:-translate-y-0.5 transition-all">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button 
                          onClick={() => handleDeleteAttendance(h.id)} 
                          variant="ghost" 
                          size="icon" 
                          title="Hapus Data"
