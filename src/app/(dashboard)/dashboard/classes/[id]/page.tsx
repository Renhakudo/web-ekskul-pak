...
export default function StudentClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async (useSkeleton = true) => {
    if (useSkeleton) setIsInitialLoading(true)
    
    // Auth user & Role cuma dipanggil sekali
    ...
    const { data: classResult } = await supabase.from('classes').select('*').eq('id', classIdRef.current).single()
    const { data: materialsResult } = await supabase.from('materials').select('*').eq('class_id', classIdRef.current).order('created_at')
    const { data: assignmentsResult } = await supabase.from('assignments').select('*').eq('class_id', classIdRef.current).order('created_at', { ascending: false })
    ...
    if (useSkeleton) setIsInitialLoading(false)
  }

  useEffect(() => { 
      fetchData(true) 
      const syncInterval = setInterval(() => { fetchData(false) }, 3000) 
  ...
  const handleCreateDiscussion = async (e: React.FormEvent) => {
    ...
    if (!error) { setNewTopicTitle(''); setNewTopicContent(''); setIsDiscussOpen(false); await fetchData(false) } 
  ...
  const handleUpdateDiscussion = async (topicId: string) => {
    ...
    if (!error) { setEditingTopicId(null); await fetchData(false) }
  ...
  const handleSendReply = async (discussionId: string) => {
    ...
    if (!error) { setReplyContent(''); await fetchData(false) } 
  ...
  const handleUpdateReply = async (replyId: string) => {
    ...
    if (!error) { setEditingReplyId(null); await fetchData(false) }
